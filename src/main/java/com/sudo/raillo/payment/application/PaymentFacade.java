...
public class PaymentFacade {
	 ...
	 * 결제 준비 처리
	 *
	 * <p>1. 예약 조회 (모든 코드 존재 확인, 소유자·만료 검증)
	 ...
	 * 결제 승인 처리
	 *
	 * <p>1. 데이터 조회 및 검증 (Order, Reservation, Member, Payment)
	...
	public PaymentConfirmResponse confirmPayment(PaymentConfirmRequest request, String memberNo) {
		...
		// 1. 데이터 조회 및 예약 유효성 검증 (만료 여부) - 예약 행을 잠가 만료 정리와 직렬화한다
		Order order = orderService.getOrderByOrderCode(request.orderId());
		List<Reservation> reservations = validateAndGetReservations(order, memberNo);
		Member member = memberService.getMemberByMemberNo(memberNo);
		...
		confirmReservations(reservations, bookingsByReservationCode);
	 ...
	 * 예약 유효성 검증 및 조회
	 *
	 * <p>결제 승인 전에 모든 예약이 점유중이며 만료되지 않았는지 확인한다.
	 * 예약 행을 잠가 만료 정리·중복 결제와 직렬화한다.</p>
	 *
	 * @throws BusinessException 예약이 없거나 일부가 만료된 경우
	 */
	private List<Reservation> validateAndGetReservations(Order order, String memberNo) {
	 ...
	 * 예약 확정 및 좌석 점유 상태 전이
	 *
	 * <p>점유 행을 옮기지 않고 HELD → CONFIRMED 로만 전이시키므로 점유가 끊기는 순간이 없다.
	 * 전이된 행 수가 기대치와 다르면 그 사이 만료·해제된 것이므로 트랜잭션이 롤백된다.</p>
	 */
	private void confirmReservations(List<Reservation> reservations, Map<String, Booking> bookingsByReservationCode) {
		for (Reservation reservation : reservations) {
			Booking booking = bookingsByReservationCode.get(reservation.getReservationCode());
			if (booking == null) {
				log.error("[예약 확정 실패] 예매를 찾을 수 없음: reservationCode={}", reservation.getReservationCode());
				throw new BusinessException(BookingError.RESERVATION_EXPIRED);
			}

			int seatCount = reservationService.countSeats(reservation.getId());
			reservation.confirm();
			seatOccupancyService.confirm(reservation, booking, seatCount);
		}
	}
