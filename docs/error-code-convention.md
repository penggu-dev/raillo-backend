...
|--------|-----|-----|-----|-----|-----|-----|
| `BOOKING_` | 예매/좌석예약 | 승차권 | 예약(Reservation) | 좌석점유·충돌 | 영수증 | — |
...
**폐기된 번호는 재사용하지 않는다.** 코드가 로그·대시보드·클라이언트 분기에 남아 있을 수 있어, 같은 번호가 다른 의미로 되살아나면 추적이 어긋난다. 밴드 안에서 계속 다음 번호를 쓴다.

예) 좌석 Hold 제거로 `BOOKING_401~404`가 폐기되었지만 신규 코드는 `BOOKING_405`부터 부여했다.
