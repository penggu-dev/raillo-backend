...
public class SeatOccupancyRepository {
	...
	/**
	 * 요청 구간에 다른 점유가 없으면 좌석을 임시 점유하고 예약을 저장한다. 검사와 쓰기가 한 스크립트에서 원자적으로 끝난다.
	 *
	 * @return 성공 또는 첫 번째 충돌 정보
	 * @throws BusinessException 스크립트 실행이 실패했거나 응답이 계약과 다를 때
	 */
	public SeatOccupancyResult hold(SeatOccupancyHoldCommand command) {
	 ...
	 */
	private static List<String> buildKeys(SeatOccupancyHoldCommand command) {
	 ...
	 */
	private static Object[] buildArgs(SeatOccupancyHoldCommand command) {
