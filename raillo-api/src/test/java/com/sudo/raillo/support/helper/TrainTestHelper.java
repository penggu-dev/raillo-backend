...
public class TrainTestHelper {
	...
	private Train saveTrainWithCarsAndSeats(Train savedTrain, List<CarSpec> carSpecs) {
		...
		List<TrainCar> trainCars = generateTrainCars(savedTrain, seatLayouts, trainTemplate);
		...
		savedTrainCars.forEach(trainCar -> {
			...
			List<Seat> seats = generateSeats(trainCar, carSpec, seatLayout);
	...
	private List<TrainCar> generateTrainCars(Train train, Map<CarType, SeatLayout> seatLayouts,
		TrainTemplate trainTemplate) {
		return java.util.stream.IntStream.range(0, trainTemplate.cars().size())
			.mapToObj(index -> {
				CarSpec spec = trainTemplate.cars().get(index);
				SeatLayout layout = seatLayouts.get(spec.carType());
				return TrainCarFixture.create(
					train,
					index + 1,
					spec.carType(),
					spec.row(),
					spec.row() * layout.columns().size(),
	...
	private List<Seat> generateSeats(TrainCar trainCar, CarSpec carSpec, SeatLayout seatLayout) {
		return java.util.stream.IntStream.rangeClosed(1, carSpec.row())
			.boxed()
			.flatMap(row -> seatLayout.columns().stream()
				.map(column -> SeatFixture.create(
					trainCar,
					row,
					column.name(),
					column.seatType(),
