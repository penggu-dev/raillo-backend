...
public class MemberUpdateService {
	...
	public SendCodeResponse requestUpdateEmail(String email, String memberNo) {
		Member member = getMember(memberNo);

		// 이미 본인 이메일이랑 동일한 이메일로 변경을 요청했을 경우 예외
		if (member.getMemberDetail().getEmail().equals(email)) {
			throw new BusinessException(MemberError.SAME_EMAIL);
		}

		// 다른 회원이 사용중인 이메일을 입력했을 경우 예외
		if (memberRepository.existsByMemberDetailEmail(email)) {
			throw new BusinessException(MemberError.DUPLICATE_EMAIL);
		}

		// 동일 요청 건이 없으면 같은 이메일에 대한 요청이 들어오지 못하도록 redis 에 등록
		if (!memberRedisRepository.handleUpdateEmailRequest(email)) {
			throw new BusinessException(MemberError.EMAIL_UPDATE_ALREADY_REQUESTED);
		}

		return emailAuthService.sendAuthCode(email);
	}

	public void verifyUpdateEmail(String newEmail, String authCode, String memberNo) {
		Member member = getMember(memberNo);

		// 이메일 검증
		if (!emailAuthService.verifyAuthCode(newEmail, authCode)) {
	 ...
	 * 회원 휴대폰 번호 변경
	 * */
	public void updatePhoneNumber(String newPhoneNumber, String memberNo) {
		Member member = getMember(memberNo);

		// 다른 회원이 사용 중인 번호를 입력했을 경우 예외
		if (memberRepository.existsByPhoneNumberAndIdNot(newPhoneNumber, member.getId())) {
	 ...
	 * 회원 비밀번호 변경
	 * */
	public void updatePassword(String newPhoneNumber, String memberNo) {
		Member member = getMember(memberNo);
		member.updatePassword(passwordEncoder.encode(newPhoneNumber));
	}
